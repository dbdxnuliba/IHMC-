...
import us.ihmc.communication.net.ROS2ObjectCommunicatorType;
...
public class PacketCommunicator implements Connectable
{
   ...
   public static PacketCommunicator createTCPPacketCommunicatorClient(String host, NetworkPorts port, NetClassList netClassList, boolean reconnectAutomatically)
   {
      ...
      if (CommunicationOptions.USE_ROS2)
      {
         ...
         objectCommunicator = new ROS2ObjectCommunicator(PubSubImplementation.FAST_RTPS, port, netClassList, ROS2ObjectCommunicatorType.CLIENT);
      }
      else
      {
                                                                      ...
                                                                      int maximumObjectSize)
   {
      ...
      {
         PrintTools.info(PacketCommunicator.class, "Creating ROS2 Fast-RTPS topic: " + port.getName(), true);
         objectCommunicator = new ROS2ObjectCommunicator(PubSubImplementation.FAST_RTPS, port, netClassList, ROS2ObjectCommunicatorType.SERVER);
      }
      else
      {
   ...
   public static PacketCommunicator createIntraprocessPacketCommunicator(NetworkPorts port, NetClassList netClassList)
   {
      ...
      if (CommunicationOptions.USE_ROS2)
      {
         ...
         objectCommunicator = new ROS2ObjectCommunicator(PubSubImplementation.INTRAPROCESS, port, netClassList, ROS2ObjectCommunicatorType.INTRAPROCESS);
      }
      else
      {
