...
//      drcSimulationTestHelper.simulateAndBlockAndCatchExceptions(0.5);
      ...
      {
         footPositions.get(robotQuadrant).setToZero(quadrupedTestFactory.getFullRobotModel().getSoleFrame(robotQuadrant));
         footPositions.get(robotQuadrant).changeFrame(ReferenceFrame.getWorldFrame());
      }
      ...
      for (int i = 1; i <= steps; i++)
      {
         ...
         for (int j = 0; j < swingOverPlanarRegionsTrajectoryExpander.getExpandedWaypoints().size(); j++)
            footstepData.getCustomPositionWaypoints().add().set(swingOverPlanarRegionsTrajectoryExpander.getExpandedWaypoints().get(j));
         ...
         stepListMessage.getQuadrupedStepList().add(frontMessage);
         stepListMessage.getQuadrupedStepList().add(hindMessage);
      ...
      conductor.addTimeLimit(variables.getYoTime(), simulationTime);
      conductor.simulate();
...
//         drcSimulationTestHelper.assertRobotsRootJointIsInBoundingBox(new BoundingBox3D(min, max));
   ...
   public void showMemoryUsageBeforeTest()
   {
      ...
      try
      {
         ...
         quadrupedTestFactory.setInitialOffset(new QuadrupedInitialOffsetAndYaw(-0.5, 0.0, 0.0, Math.PI));
