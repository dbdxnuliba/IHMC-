...
//      CombinedTerrainObject3D profile3D = createStepDownsAndFlatsTerrain();
      ...
      for (int i = 0; i < numberOfFlamingos; i++)
      {
         ...
         if (controllerToUse == BALLISTIC_WALKING_CONTROLLER)
         {
            gcModel = new LinearGroundContactModel(springFlamingo, 14220, 150.6, 125.0, 300.0, springFlamingo.getRobotsYoVariableRegistry());
         }
         else if (controllerToUse == FAST_WALKING_CONTROLLER)
         {
            gcModel = new LinearGroundContactModel(springFlamingo, 50000.0, 2500.0, 100.0, 500.0, springFlamingo.getRobotsYoVariableRegistry());
      ...
      else
      {
         ...
         sim = new SimulationConstructionSet(springFlamingos, new SimulationConstructionSetParameters(initialBufferSize));
      }
...
//      sim.setSimulateNoFasterThanRealTime(true);
      ...
      sim.setupEntryBoxGroup("control vars1", new String[] {"t_gain", "t_damp", "hip_d", "hip_hold", "hip_down", "hip_gain", "hip_damp", "swing_gain_knee",
            "swing_damp_knee", "q_x", "q_y", "q_z"});
      ...
      else if (RUN_AND_COMPARE)
      {
         ...
         ArrayList<VariableDifference> variableDifferences = StateFileComparer.absoluteCompareStateFiles("simAfter5Seconds.state", "newSimAfter5Seconds.state",
                                                                                                         0.001, null);
   ...
   private static void setupCamerasAndViewports(SimulationConstructionSet sim)
   {
