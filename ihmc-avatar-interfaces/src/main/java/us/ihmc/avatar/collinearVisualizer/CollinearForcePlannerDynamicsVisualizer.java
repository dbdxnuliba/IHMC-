...
public abstract class CollinearForcePlannerDynamicsVisualizer
{
   ...
   private final SideDependentList<YoFramePose3D> solePose = new SideDependentList<>();
   ...
   private final ArrayList<YoFramePoseUsingYawPitchRoll> contactStateLabels = new ArrayList<>();
   ...
   private final ArrayList<GraphicsUpdatable> updatableGraphicsList = new ArrayList<>();
   ...
   private PlaybackListener createContactStateGraphics()
   {
      ...
      for (int i = 0; i < maxNumberOfContactStatesToVisualize; i++)
      {
         ...
         YoFramePoseUsingYawPitchRoll textPose = new YoFramePoseUsingYawPitchRoll("TextPose" + i, worldFrame, graphicsSubRegistry);
      ...
      // Just to not have weird typecasting
      for(int i = 0; i < contactStateViz.size(); i++)
         updatableGraphicsList.add(contactStateViz.get(i));
      return new GraphicsUpdatablePlaybackListener(updatableGraphicsList);
   }
   ...
   private void getFootSupportPolygonAndCreateSoleFrames(DRCRobotJointMap jointMap, RobotDescription robotDescription)
   {
      for (RobotSide side : RobotSide.values)
      {
         ...
         YoFramePose3D soleFramePose = new YoFramePose3D(namePrefix + sideName + "SolePose", worldFrame, registry);
   ...
   private void createFeetGraphics(DRCRobotJointMap jointMap, RobotDescription robotDescription)
   {
      ...
      {
         String sideName = side.getCamelCaseNameForMiddleOfExpression();
         YoFramePoseUsingYawPitchRoll footPose = new YoFramePoseUsingYawPitchRoll(namePrefix + sideName + "FootPose", worldFrame, graphicsSubRegistry);
         String jointBeforeFootName = jointMap.getJointBeforeFootName(side);
         ...
         YoGraphicShape footGraphic = new YoGraphicShape(namePrefix + sideName + "FootViz", footGraphicObject, footPose, 1.0);
         YoFramePose3D soleFramePose = solePose.get(side);
         YoFrameYawPitchRoll soleFrameYawPitchRoll = new YoFrameYawPitchRoll(namePrefix + sideName + "SoleOrientationInEulerAngles", worldFrame, graphicsSubRegistry);
         soleFramePose.getOrientation().attachVariableChangedListener(new VariableChangedListener()
         {
            
            @Override
            public void notifyOfVariableChange(YoVariable<?> v)
            {
               soleFrameYawPitchRoll.set(soleFramePose.getOrientation());
            }
         });
         YoFramePoseUsingYawPitchRoll soleFramePoseUsingYawPitchRoll = new YoFramePoseUsingYawPitchRoll(soleFramePose.getPosition(), soleFrameYawPitchRoll);
         YoGraphicCoordinateSystem footFrameGraphic = new YoGraphicCoordinateSystem(side.getCamelCaseName() + "FootFrameGraphic", soleFramePoseUsingYawPitchRoll, 0.1);
         soleFramePose.attachVariableChangedListener(new VariableChangedListener()
         {
            ...
            public void notifyOfVariableChange(YoVariable<?> v)
            {
               soleFramePose.get(tempPointForPoseSetting, tempOrientationForPoseSetting);
               tempPose.setIncludingFrame(tempPointForPoseSetting.getReferenceFrame(),tempPointForPoseSetting, tempOrientationForPoseSetting);
   ...
   private List<FramePoint3D> supplierSpoofReference;
   private final FrameVertex2DSupplier supplierSpoof = new FrameVertex2DSupplier()
   {
      private final FramePoint2D pointForProjection = new FramePoint2D();
      @Override
      public FramePoint2DReadOnly getVertex(int index)
      {
         pointForProjection.setIncludingFrame(supplierSpoofReference.get(index));
   ...
   private void updateContactStateVisualization(List<ContactState> contactStatesToVisualize)
   {
      ...
      for (i = 0; i < numberOfContactStatesToViz; i++)
      {
         ...
         supplierSpoofReference = framePointList;
         tempFramePolygon.set(supplierSpoof);
         framePointList.clear();
   ...
   private void prepareRunningContactStatePlan()
   {
      ...
      Vector2D stepSize = new Vector2D(0.8, 0.);
      solePose.get(RobotSide.LEFT).get(tempPointForPoseSetting, tempOrientationForPoseSetting);
      tempPose.setIncludingFrame(tempPointForPoseSetting.getReferenceFrame(),tempPointForPoseSetting, tempOrientationForPoseSetting);
      FramePose2D leftSolePose = new FramePose2D(tempPose);
      solePose.get(RobotSide.RIGHT).get(tempPointForPoseSetting, tempOrientationForPoseSetting);
      tempPose.setIncludingFrame(tempPointForPoseSetting.getReferenceFrame(),tempPointForPoseSetting, tempOrientationForPoseSetting);
      FramePose2D rightSolePose = new FramePose2D(tempPose);
      contactStatePlanner.generateContactStatePlanForRunning(contactStatePlan, leftSolePose, rightSolePose, stepSize, numberOfSteps,
                                                             feetSupportPolygon.get(RobotSide.LEFT), feetSupportPolygon.get(RobotSide.RIGHT), RobotSide.LEFT,
                                                             true, false, true, false, 0.2, 0.4, 0.4);
   }

   private final FramePoint3D tempPointForPoseSetting = new FramePoint3D();
   private final FrameQuaternion tempOrientationForPoseSetting = new FrameQuaternion();
   
   private void prepareWalkingContactStatePlan()
   {
      int numberOfSteps = 3;
      createContactStates(numberOfSteps * 2 + 1);
      Vector2D stepSize = new Vector2D(0.4, 0.0);
      solePose.get(RobotSide.LEFT).get(tempPointForPoseSetting, tempOrientationForPoseSetting);
      tempPose.setIncludingFrame(tempPointForPoseSetting.getReferenceFrame(),tempPointForPoseSetting, tempOrientationForPoseSetting);
      FramePose2D leftSolePose = new FramePose2D(tempPose);
      solePose.get(RobotSide.RIGHT).get(tempPointForPoseSetting, tempOrientationForPoseSetting);
...
//         PrintTools.debug("State: " + i + ": " + contactStatePlan.get(i).toString());
   ...
   private void prepareJumpingContactStatePlan()
   {
      ...
      YoFramePose3D yoLeftPose = solePose.get(RobotSide.LEFT);
      FramePose2D leftFootPose = new FramePose2D();
      leftFootPose.setIncludingFrame(yoLeftPose.getReferenceFrame(), yoLeftPose.getX(), yoLeftPose.getY(), yoLeftPose.getYaw());
      YoFramePose3D yoRightPose = solePose.get(RobotSide.RIGHT);
      FramePose2D rightFootPose = new FramePose2D();
      rightFootPose.setIncludingFrame(yoRightPose.getReferenceFrame(), yoRightPose.getX(), yoRightPose.getY(), yoRightPose.getYaw());
      contactStatePlanner.generateContactStatePlanForJumping(contactStatePlan, leftFootPose, rightFootPose, stepSize, numberOfJumps,
                                                             feetSupportPolygon.get(RobotSide.LEFT), feetSupportPolygon.get(RobotSide.RIGHT), 0.15, 0.5);
   }
}