...
public class CollinearForceVisualizationController extends CentroidalRobotController
{
   ...
   private YoFramePoint3D desiredCoP;
   private YoFrameVector3D desiredGroundReactionForce;

   private final SideDependentList<YoFramePose3D> solePoses;
   ...
   // YoVariables 
   private final YoFramePoint3D plannedCoM;
   private final YoFramePoint3D plannedCoP;
   private final YoFramePoint3D dynamicsCoM;
   private final YoFramePoint3D dynamicsCoP;
   ...
   public CollinearForceVisualizationController(SideDependentList<YoFramePose3D> solePoses, SideDependentList<ConvexPolygon2D> feetSupportPolygonInSoleFrame,
                                                YoGraphicsListRegistry graphicsListRegistry)
   {
      ...
      this.gravity = new YoFrameVector3D(namePrefix + "Gravity", worldFrame, registry);
      ...
      plannedCoM = new YoFramePoint3D("PlannedCoM", worldFrame, registry);
      plannedCoP = new YoFramePoint3D("PlannedCoP", worldFrame, registry);
      dynamicsCoM = new YoFramePoint3D("DynamicsCoM", worldFrame, registry);
      dynamicsCoP = new YoFramePoint3D("DynamicsCoP", worldFrame, registry);
   ...
   private void updateYoVariables()
   {
      for (RobotSide side : RobotSide.values)
      {
         tempFramePolygon.clear();
         YoFramePose3D solePose = solePoses.get(side);

         footOnGround.get(side).set(solePose.getZ() < 1e-4);
         if (footOnGround.get(side).getBooleanValue())
         {
            tempFramePolygon.setIncludingFrame(solePose.getReferenceFrame(), defaultSupportPolygons.get(side));
            TransformHelperTools.transformFromPoseToReferenceFrameByProjection(solePose, tempFramePolygon);
            tempFramePolygon.changeFrame(worldFrame);
            feetPolygonsInWorldFrame.get(side).set(tempFramePolygon);
         }
      }
      tempFramePolygon.clear();
      for (RobotSide side : RobotSide.values)
      {
         if (footOnGround.get(side).getBooleanValue())
            tempFramePolygon.addVertices(feetPolygonsInWorldFrame.get(side));
      }
      tempFramePolygon.update();
      netSupportPolygon.set(tempFramePolygon);
   }
   ...
   private void runControllerCore(FramePoint3DReadOnly centerOfMass, FrameVector3D linearMomentumRateOfChange, FrameVector3D angularMomentumRateOfChange)
   {
      ...
      tempFramePolygon.setIncludingFrame(worldFrame, tempPolygon);
      controllerCore.setSupportPolygon(tempFramePolygon);
   ...
   @Override
   public void setOutputVariables(YoFramePoint3D desiredCoPToSet, YoFrameVector3D desiredExternalForce)
   {
