...
public class CentroidalRobotControllerCore
{
   ...
   private final YoFrameVector3D achievedLinearMomentumRateOfChange;
   private final YoFrameVector3D achievedAngularMomentumRateOfChange;

   // Optimization inputs
   private final YoFramePoint3D currentCenterOfMass;
   private final YoFrameVector3D linearMomentumWeights;
   private final YoFrameVector3D angularMomentumWeights;
   private YoDouble mass;
   private YoFrameVector3D gravity;
   private final YoFrameVector3D groundReactionWeights;
   private final YoFrameVector3D desiredLinearMomentumRateOfChange;
   private final YoFrameVector3D desiredAngularMomentumRateOfChange;
   private final SideDependentList<YoBoolean> footOnGround = new SideDependentList<>();
   private final SideDependentList<YoFramePoint3D> ankleLocation = new SideDependentList<>();
                                        ...
                                        YoGraphicsListRegistry graphicsListRegistry)
   {
      ...
      achievedLinearMomentumRateOfChange = new YoFrameVector3D(namePrefix + "AchievedLinearMomentumRateOfChange", worldFrame, registry);
      achievedAngularMomentumRateOfChange = new YoFrameVector3D(namePrefix + "AchievedAngularMomentumRateOfChange", worldFrame, registry);
      failureFlag = new YoBoolean(namePrefix + "FailureFlag", registry);
      currentCenterOfMass = new YoFramePoint3D(namePrefix + "CenterOfMass", worldFrame, registry);
      desiredLinearMomentumRateOfChange = new YoFrameVector3D(namePrefix + "DesiredLinearMomentumRateOfChange", worldFrame, registry);
      desiredAngularMomentumRateOfChange = new YoFrameVector3D(namePrefix + "DesiredAngularMomentumRateOfChange", worldFrame, registry);
      ...
      for (RobotSide side : RobotSide.values)
      {
         ...
         YoFramePoint3D ankleLocation = new YoFramePoint3D(namePrefix + side.getCamelCaseNameForMiddleOfExpression() + "AnkleLocation", worldFrame, registry);
      ...
      numberOfSupportPolygonVertices = supportPolygon.getYoNumberOfVertices();
      enableGroundReactionLimits = new YoBoolean(namePrefix + "EnableGroundReactionForceLimits", registry);
      groundReactionWeights = new YoFrameVector3D(namePrefix + "GroundReactionWeights", worldFrame, registry);
      linearMomentumWeights = new YoFrameVector3D(namePrefix + "LinearMomentumOptimizationWeights", worldFrame, registry);
      angularMomentumWeights = new YoFrameVector3D(namePrefix + "AngularMomentumOptimizationWeights", worldFrame, registry);
      ...
      if (graphicsListRegistry != null)
      {
         ...
         List<YoFramePoint2D> supportPolygonVertices = supportPolygon.getVertexBuffer();
         for (int i = 0; i < maxNumberOfVertices; i++)
         {
            YoFramePoint2D vertex = supportPolygonVertices.get(i);
            YoFrameVector3D forceVector = new YoFrameVector3D(namePrefix + "ForceVector" + i, worldFrame, registry);
   ...
   public void setSupportPolygon(FrameConvexPolygon2DReadOnly supportPolygonToSet)
   {
      supportPolygon.set(supportPolygonToSet);
   }
   ...
   private void updateSupportPolygon()
   {
      ...
      supportPolygon.set(supportPolygonForComputation);
   }
   ...
   private void generateDynamicsConstraint()
   {
      ...
      {
         tempV.set(supportPolygon.getVertex(i));
   ...
   public void setGravity(YoFrameVector3D gravity)
   {
