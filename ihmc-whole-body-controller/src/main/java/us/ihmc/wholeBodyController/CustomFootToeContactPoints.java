...
public class CustomFootToeContactPoints implements FootContactPoints {
	...
	public Map<String, List<Tuple3DBasics>> getSimulationContactPoints(double footLength, double footWidth,
		...
		// considering only half foot for 4 contact points
		footLength = footLength/2;
		RigidBodyTransform toeTransform = soleToAnkleFrameTransforms.get(RobotSide.LEFT);
		
		//hack
		toeTransform.setTranslation(soleToAnkleFrameTransforms.get(RobotSide.LEFT).getTranslationX()+toeWidth, soleToAnkleFrameTransforms.get(RobotSide.LEFT).getTranslationY(), soleToAnkleFrameTransforms.get(RobotSide.LEFT).getTranslationZ());
		Double xTranslation = soleToAnkleFrameTransforms.get(RobotSide.LEFT).getTranslationX();
		Double offset = 0.03;
		soleToAnkleFrameTransforms.get(RobotSide.LEFT).setTranslationX(xTranslation - offset);;
		for (RobotSide robotSide : RobotSide.values)
	      {
	         String toeJointName = jointMap.getLegJointName(robotSide, LegJointName.TOE_PITCH);
	         String ankleJointName = jointMap.getLegJointName(robotSide, LegJointName.ANKLE_PITCH);
	         ...
	         for (int ix = 1; ix <= nContactPointsX; ix++)
	         {
	            ...
	            double footWidthAtCurrentX = (1.0 - alpha) * 1.01 * footWidth + alpha * 1.01 * footWidth;
	            ...
	            for (int iy = 1; iy <= nContactPointsY; iy++)
	            {
	               ...
	               Point3D contactPointFoot = new Point3D(x, y, 0.0);
	               transformToParentJointFrame.transform(contactPointFoot);
	               
	               Point3D contactPointToe = new Point3D(x, y, 0.0);
	               toeTransform.transform(contactPointToe);
	               
	               footContactPoints.add(contactPointFoot);
	               toeContactPoints.add(contactPointToe);
	            }
	         }
