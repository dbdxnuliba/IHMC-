package us.ihmc.simulationConstructionSetTools.util.environments.planarRegionEnvironments;

import us.ihmc.graphicsDescription.appearance.YoAppearance;

public class WideWallsWithIncreasingHeightPlanarRegionEnvironment extends PlanarRegionEnvironmentInterface
{
   public WideWallsWithIncreasingHeightPlanarRegionEnvironment()
   {
      generator.translate(2.0, 0.0, -0.01);
      generator.addCubeReferencedAtBottomMiddle(6.0, 1.0, 0.01);
      generator.translate(-2.0, 0.0, 0.0);
      generator.translate(0.35, 0.0, 0.0);
      generator.addCubeReferencedAtBottomMiddle(0.1, 1.0,  0.1);
      generator.translate(0.62, 0.0, 0.0);
      generator.addCubeReferencedAtBottomMiddle(0.1, 1.0, 0.14);
      generator.translate(0.3, 0.0, 0.0);
      generator.addCubeReferencedAtBottomMiddle(0.1, 1.0, 0.15);
      generator.translate(0.4, 0.0, 0.0);
      generator.addCubeReferencedAtBottomMiddle(0.1, 1.0, 0.11);
      addPlanarRegionsToTerrain(YoAppearance.Grey());
   }
}
