...
public class SmoothCapturePointToolboxTest
{
   ...
   public void testComputeDesiredCornerPointsFromCMPPolynomials()
   {
      ...
      for (int j = 0; j < nTests; j++)
      {
         ...
         for (int i = numberOfCoPWaypoints - 2; i >= 0; i--)
         {
            ...
            SmoothCapturePointToolboxTest.calculateICPPositionByHand3DLinear(omega0, time, cmpPolynomials3D.get(i), exitCornerPointsByHandToPack.get(i),
                                                                             newEntryICP);
   ...
   public static void calculateICPPositionByHand3DLinear(double omega0, double time, FrameTrajectory3D linear3D, FramePoint3D icpPositionDesiredFinal,
                                                         FramePoint3D icpPositionDesiredCurrent)
   {
   ...
   public static void calculateICPPositionByHand3DCubic(double omega0, double time, FrameTrajectory3D cubic3D, FramePoint3D icpPositionDesiredFinal,
                                                        FramePoint3D icpPositionDesiredCurrent)
   {
   ...
   public static void calculateICPVelocityByHand3DLinear(double omega0, double time, FrameTrajectory3D linear3D, FramePoint3D icpPositionDesiredFinal,
                                                         FrameVector3D icpVelocityDesiredCurrent)
   {
      linear3D.compute(linear3D.getInitialTime());
      FramePoint3D cmpRefInit = new FramePoint3D(linear3D.getFramePosition());
   ...
   public static void calculateICPVelocityByHand3DCubic(double omega0, double time, FrameTrajectory3D cubic3D, FramePoint3D icpPositionDesiredFinal,
                                                        FrameVector3D icpVelocityDesiredCurrent)
   {
      cubic3D.compute(cubic3D.getInitialTime());
      FramePoint3D cmpRefInit = new FramePoint3D(cubic3D.getFramePosition());
   ...
   public static void calculateBetaPrime3DByHandCubic(double omega0, double time, double timeTotal, DenseMatrix64F betaPrimeLinear)
   {
      ...
      betaPrimeLinear.set(0, 3, Math.exp(omega0 * (time - timeTotal))
            * (Math.pow(timeTotal, 3) + 3.0 * Math.pow(timeTotal, 2) / omega0 + 6.0 * timeTotal / Math.pow(omega0, 2) + 6.0 / Math.pow(omega0, 3)));
      ...
      betaPrimeLinear.set(1, 7, Math.exp(omega0 * (time - timeTotal))
            * (Math.pow(timeTotal, 3) + 3.0 * Math.pow(timeTotal, 2) / omega0 + 6.0 * timeTotal / Math.pow(omega0, 2) + 6.0 / Math.pow(omega0, 3)));
      ...
      betaPrimeLinear.set(2, 11, Math.exp(omega0 * (time - timeTotal))
            * (Math.pow(timeTotal, 3) + 3.0 * Math.pow(timeTotal, 2) / omega0 + 6.0 * timeTotal / Math.pow(omega0, 2) + 6.0 / Math.pow(omega0, 3)));
   }
   ...
   @Test(timeout = 3000)
   public void testOmegaPowersComputation()
   {
      DenseMatrix64F omega0Powers = new DenseMatrix64F(1, 0);
      int n = 10;
      SmoothCapturePointToolbox.calculateOmegaPowers(omega0, omega0Powers, n);
      assertEquals(omega0Powers.numRows, 1);
      assertEquals(omega0Powers.numCols, n);
      assertEquals(omega0Powers.get(0, 0), 1.0, Epsilons.ONE_BILLIONTH);
      assertEquals(omega0Powers.get(0, 1), 1.0 / omega0, Epsilons.ONE_BILLIONTH);
      assertEquals(omega0Powers.get(0, n - 1), Math.pow(omega0, -n + 1), Epsilons.ONE_BILLIONTH);
   }

   @Test(timeout = 3000)
   public void testTimeDerivativeMatrix()
   {
      int n = 10;
      DenseMatrix64F derivativeCoefficientMatrix = SmoothCapturePointToolbox.generateDerivativeCoefficientMatrix(n);
      assertEquals(derivativeCoefficientMatrix.numCols, n);
      assertEquals(derivativeCoefficientMatrix.numRows, n);
   }

   @Test(timeout = 3000)
   public void compareAlphaPrimeComputation()
   {
      Trajectory traj = new Trajectory(10);
      traj.setInitialTime(0.5);
      traj.setFinalTime(3.0);
      traj.setDirectly(9, 0.0);
      ...
      SmoothCapturePointToolbox.calculateGeneralizedAlphaPrimeOnCMPSegment1D(omega0, traj.getInitialTime(), fromOldCalc, 0, traj);
      ...
      SmoothCapturePointToolbox.calculateTimePowers(timeDerivativePowers, traj.getInitialTime(), 10, timeDerivativeCoefficients);
      DenseMatrix64F omegaPowers = new DenseMatrix64F(0, 1);
      SmoothCapturePointToolbox.calculateOmegaPowers(omega0, omegaPowers, 10);
      SmoothCapturePointToolbox.calculateAlphaPrimeMatrix(fromNewCalc, 10, timeDerivativePowers, omegaPowers);
      PrintTools.debug("From Old: " + fromOldCalc.toString());
      PrintTools.debug("From New: " + fromNewCalc.toString());
   }

   @Test(timeout = 3000)
   public void compareBetaPrimeComputation()
   {
      Trajectory traj = new Trajectory(10);
      traj.setInitialTime(1.0);
      traj.setFinalTime(3.0);
      traj.setDirectly(9, 0.0);
      traj.setDirectlyFast(0, 1.0);
      DenseMatrix64F fromOldCalc = new DenseMatrix64F(0, 1);
      DenseMatrix64F fromNewCalc = new DenseMatrix64F(0, 1);
      SmoothCapturePointToolbox.calculateGeneralizedBetaPrimeOnCMPSegment1D(omega0, 2.0, fromOldCalc, 0, traj);
      DenseMatrix64F timeDerivativeCoefficients = SmoothCapturePointToolbox.generateDerivativeCoefficientMatrix(10);
      DenseMatrix64F finalTimeDerivativePowers = new DenseMatrix64F(0, 1);
      SmoothCapturePointToolbox.calculateTimePowers(finalTimeDerivativePowers, traj.getFinalTime(), 10, timeDerivativeCoefficients);
      DenseMatrix64F omegaPowers = new DenseMatrix64F(0, 1);
      SmoothCapturePointToolbox.calculateOmegaPowers(omega0, omegaPowers, 10);
      SmoothCapturePointToolbox.calculateBetaPrimeMatrix(fromNewCalc, 10, finalTimeDerivativePowers, omegaPowers,
                                                         SmoothCapturePointToolbox.calculateGammaPrimeMatrix(omega0, 2.0, traj.getFinalTime()));
      PrintTools.debug("From Old: " + fromOldCalc.toString());
      PrintTools.debug("From New: " + fromNewCalc.toString());
   }

   @Test(timeout = 1000)
   public void testGammaPrime()
   {
      Trajectory traj = new Trajectory(10);
      traj.setInitialTime(1.0);
      traj.setFinalTime(3.0);
      traj.setDirectly(9, 0.0);
      traj.setDirectlyFast(0, 1.0);
      PrintTools.debug("From old: " + SmoothCapturePointToolbox.calculateGeneralizedGammaPrimeOnCMPSegment1D(omega0, 2.0, 0, traj));
      PrintTools.debug("From new: " + SmoothCapturePointToolbox.calculateGammaPrimeMatrix(omega0, 2.0, traj.getFinalTime()));
   }

   @Test(timeout = 1000)
   public void testCapturePointComputation()
   {
      Trajectory traj = new Trajectory(10);
      traj.setInitialTime(1.5);
      traj.setFinalTime(3.0);
      traj.setDirectly(9, 0.0);
      traj.setDirectlyFast(0, 2.0);
      SmoothCapturePointToolbox capturePointToolbox = new SmoothCapturePointToolbox(10);
      double icpPositionDesiredFinal = 3.0;
      //double fromNew = capturePointToolbox.calculateICPQuantityFromCorrespondingCMPPolynomial1D(omega0, 2.0, traj, icpPositionDesiredFinal);
      double fromOld = capturePointToolbox.calculateICPQuantityFromCorrespondingCMPPolynomial1D(omega0, 2.0, 0, traj, icpPositionDesiredFinal);
      //PrintTools.debug("From old: " + fromOld + ", From new: " + fromNew);
   }
}
