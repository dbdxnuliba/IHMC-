...
public class FootstepAngularMomentumPredictor implements AngularMomentumTrajectoryGeneratorInterface
{
   ...
   private final YoDouble swingFootNominalLift;
   ...
   private final Trajectory segmentSwingFootLiftTrajectory;
   ...
   public FootstepAngularMomentumPredictor(String namePrefix, YoDouble omega0, boolean debug, YoVariableRegistry parentRegistry)
   {
      ...
      this.swingFootNominalLift = new YoDouble(fullPrefix + "SwingFootNominalLift", registry);
      ...
      this.segmentSwingFootLiftTrajectory = new Trajectory(maxNumberOfTrajectoryCoefficients);
   ...
   public void initializeParameters(SmoothCMPPlannerParameters smoothCMPPlannerParameters, double totalMass, double gravityZ)
   {
      ...
      this.swingFootNominalLift.set(angularMomentumParameters.getSwingFootMaxLift());
   ...
   private void setCoMTrajectory(double initialTime, double finalTime, int comIndex)
   {
      ...
      segmentCoMPositionTrajectory.setQuintic(initialTime, finalTime, tempFramePoint1, comInitialVelocities.get(comIndex),
   ...
   private void setSwingFootTrajectoryForPhase(int footstepIndex, WalkingTrajectoryType phase, double phaseDuration)
   {
      if (phase == WalkingTrajectoryType.SWING)
      {
         ...
         segmentSwingFootPositionTrajectory.setQuinticWithZeroTerminalAcceleration(0.0, phaseDuration, tempFramePoint1, zeroVector, tempFramePoint2,
                                                                                   zeroVector);
         addSwingLiftTrajectory(segmentSwingFootPositionTrajectory.getTrajectoryZ(), phaseDuration);
      }
      else
      {
         upcomingCoPsInFootsteps.get(footstepIndex).getSupportFootLocation(tempFramePoint1);
         segmentSwingFootPositionTrajectory.setQuinticWithZeroTerminalAcceleration(0.0, phaseDuration, tempFramePoint1, zeroVector, tempFramePoint1,
                                                                                   zeroVector);
      }
   }

   private void addSwingLiftTrajectory(Trajectory swingFootZTrajectory, double phaseDuration)
   {
      segmentSwingFootLiftTrajectory.setQuadraticUsingIntermediatePoint(0.0, phaseDuration * 0.5, phaseDuration, 0.0, swingFootNominalLift.getDoubleValue(),
                                                                        0.0);
      TrajectoryMathTools.add(swingFootZTrajectory, swingFootZTrajectory, segmentSwingFootLiftTrajectory);
   }
