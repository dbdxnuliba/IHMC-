...
public class FootstepPathCalculatorModule
{
   ...
   public FootstepPathCalculatorModule(Messager messager)
   {
      ...
      parameters = messager.createInput(PlannerParametersTopic, new BipedFootstepPlannerParameters());
   ...
   private BodyPathAndFootstepPlanner createAStarPlanner(SideDependentList<ConvexPolygon2D> footPolygons, YoVariableRegistry registry)
   {
      ...
      DistanceAndYawBasedHeuristics heuristics = new DistanceAndYawBasedHeuristics(parameters.getCostParameters().getAStarHeuristicsWeightProvider(), parameters);
